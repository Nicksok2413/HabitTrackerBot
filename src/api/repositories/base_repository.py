...
from src.api.models.base import Base as SQLAlchemyBaseModel
...
class BaseRepository(Generic[ModelType, CreateSchemaType, UpdateSchemaType]):
    ...
    async def get_by_filter_first_or_none(
        ...
        Returns:
            ModelType | None: Экземпляр модели или None, если запись не найдена.
        """
        log.debug(f"Получение первого {self.model.__name__} по фильтрам: {filters}")
        ...
        if instance:
            log.debug(f"Найден {self.model.__name__} по фильтрам.")
        else:
            log.debug(f"{self.model.__name__} по указанным фильтрам не найден.")
    ...
    async def get_multi(
        ...
        """
        log.debug(
            f"Получение списка {self.model.__name__} (skip={skip}, limit={limit})"
        )
    ...
    async def get_multi_by_filter(
        ...
        *filters: ColumnElement[bool],
        ...
        instances = result.scalars().all()
        log.debug(
            f"Найдено {len(instances)} записей {self.model.__name__} по фильтрам."
        )
        return instances
    ...
    async def update(
        ...
        *,
        ...
        if isinstance(obj_in, BaseModel):
            # exclude_unset=True важен для частичного обновления
        ...
        for field, value in update_data.items():
            if hasattr(db_obj, field):
                ...
                log.warning(
                    f"Попытка обновить несуществующее поле '{field}' для {self.model.__name__} ID: {db_obj.id}"
                )
    ...
    async def remove(
        ...
        Args:
            db_session (AsyncSession): Асинхронная сессия базы данных.
            obj_id (int): Идентификатор записи для удаления.

        Returns:
            ModelType | None: Объект, помеченный для удаления, или None, если не найден.
        """
        log.debug(f"Подготовка к удалению {self.model.__name__} по ID: {obj_id}")
        db_obj = await self.get_by_id(db_session, obj_id=obj_id)

        if db_obj:
            await self.delete(db_session, db_obj=db_obj)
