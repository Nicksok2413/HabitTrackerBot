...
class UserSchemaBase(BaseSchema):
    ...
    first_name: str | None = Field(
        None, max_length=100, description="Имя пользователя в Telegram"
    )
...
class UserSchemaCreate(UserSchemaBase):
    ...
    telegram_id: int = Field(
        ..., gt=0, description="Уникальный идентификатор пользователя в Telegram"
    )
...
class UserSchemaUpdate(BaseSchema):
    ...
    username: str | None = Field(
        None, max_length=100, description="Новый Username пользователя в Telegram"
    )
    first_name: str | None = Field(
        None, max_length=100, description="Новое имя пользователя"
    )
    last_name: str | None = Field(
        None, max_length=100, description="Новая фамилия пользователя"
    )
    is_active: bool | None = Field(None, description="Статус активности пользователя")
    is_bot_blocked: bool | None = Field(
        None, description="Статус блокировки бота пользователем"
    )
...
class UserSchemaRead(UserSchemaBase):
    ...
    is_bot_blocked: bool = Field(
        ..., description="Статус блокировки бота пользователем"
    )
