...
from src.api.models.habit_execution import HabitExecutionStatus  # Импортируем Enum
...
class HabitExecutionSchemaBase(BaseSchema):
    ...
    execution_date: date = Field(
        ..., description="Дата, на которую зафиксировано выполнение"
    )
    status: HabitExecutionStatus = Field(..., description="Статус выполнения привычки")


class HabitExecutionSchemaCreate(
    BaseSchema
):  # Не наследуем от HabitExecutionSchemaBase
...
class HabitExecutionSchemaUpdate(BaseSchema):
    ...
    status: HabitExecutionStatus = Field(
        ..., description="Новый статус выполнения привычки"
    )
...
class HabitExecutionSchemaRead(HabitExecutionSchemaBase):
    ...
    habit_id: int = Field(
        ..., description="ID привычки, к которой относится выполнение"
    )
    # execution_date и status наследуются
    created_at: datetime = Field(..., description="Время создания записи о выполнении")
    updated_at: datetime = Field(
        ..., description="Время последнего обновления записи о выполнении"
    )
